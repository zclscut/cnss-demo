...
%     t=0:1/fs_music:(N-1)/fs_music;
    ...
    %若不够两位则补0
    sym_trans=[dec2hex(pcm1(i+1),2) dec2hex(pcm1(i+2),2)];%要画出的传输码元transmitted symbols
    set(handles.edit10,'String',sym_trans);%输出 41(dec)=0x29(hex),65(dec)=0x41(hex)
    ...
    if get(handles.radiobutton1,'Value')==1
        ...
        title('发射模拟信号');
        yyaxis right
        sym_trans=[floor(pcm1(i+1)/16),mod(pcm1(i+1),16),floor(pcm1(i+2)/16),mod(pcm1(i+2),16)];
        stem(1:4,sym_trans,'r',ShowBaseLine='off');
        legend('','码元')
...
%         tt=cputime;%程序计时变量
        ...
        title('接收模拟信号');
        yyaxis right
        sym_trans=[floor(pcm1_rc(i+1)/16),mod(pcm1_rc(i+1),16),floor(pcm1_rc(i+2)/16),mod(pcm1_rc(i+2),16)];
        stem(1:4,sym_trans,'r',ShowBaseLine='off');
        legend('','码元')
...
%         disp(['计算误码率总耗时:',num2str(tt),'秒'])%程序计时变量
    ...
    elseif get(handles.radiobutton3,'Value')==1
        ...
        axes(handles.axes1);
        yyaxis left
        plot(t1,qam16(i*(1/tao*2)+1:(i+2)*(1/tao*2)));
        hold on
        xlabel('时间/s');
        title('发射模拟信号');
        yyaxis right
